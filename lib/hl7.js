...
(function(module, process, console){
	...
	class HL7Segment {
		...
		toMappedObject(compact) {
			if (typeof this.segmentsFields[this.typeofSegment] === 'object') {
				...
				var obj = {}, fields = this.segmentsFields[this.typeofSegment];
	...
	class hl7Parser {
		...
		parseFile(path, wrappedDone) {
			var self = this, fileEncoding = self.options.fileEncoding;
			...
			self.options.fs.stat(path, function (err, stats) {
				...
				self.options.fs.open(path, 'r', function (erro, fd) {
					...
					var readBuffer = Buffer.alloc(size), bufferOffset = 0, bufferLength = readBuffer.length, filePosition = 0;
